...
Connor \& van Leeuwen (2018) introdujeron uno de los primeros enfoques de este tipo, entrenando una red neuronal convolucional para asignar a cada candidato una probabilidad de ser una señal astrofísica real.
...
\subsubsection{El sistema FETCH: clasificación automática de candidatos}

Agarwal \textit{et al}. (2020) desarrollaron el sistema \textbf{FETCH} (\textit{Fast Extragalactic Transient Candidate Hunter}), que consiste en una serie de modelos de deep learning (redes neuronales profundas) entrenados con ejemplos de FRBs simulados y datos reales de RFI.
...
\subsubsection{Otros pipelines modernos: DDSS}
...
\subsubsection{Ventajas y desafíos de los métodos de deep learning}
...
Sin embargo, también presentan desafíos importantes:

\textbf{Desafíos técnicos:} (i) las \textit{señales débiles y difusas} (con S/N bajo, o muy ensanchadas temporalmente) pueden pasar inadvertidas para una red entrenada principalmente con ejemplos más brillantes, pues en la imagen de entrada se ven muy poco contrastadas \cite{zhang2024drafts}.
...
\subsubsection{El pipeline DRAFTS: detección y clasificación integradas}
...
\subsubsection{Arquitectura del sistema DRAFTS}
...
\subsubsection{Rendimiento y casos de estudio de DRAFTS}

Los resultados reportados para este pipeline muestran mejoras notables frente a los métodos tradicionales. En pruebas con datos simulados con inyecciones de pulsos, la fase de detección con CenterNet logró una \textit{tasa de recuperación} (recall) de \textasciitilde{}96--97\% con una tasa de falsas detecciones muy baja (\textasciitilde{}20--23 falsos en total), superando ampliamente el recall de \textasciitilde{}85\% de PRESTO en condiciones equivalentes \cite{zhang2024drafts}.
...
\subsubsection{Resumen y perspectivas futuras}

Las técnicas modernas basadas en \textit{deep learning} están logrando reducir drásticamente los falsos positivos, automatizar la selección de eventos astrofísicos y aumentar la sensibilidad de detección. La combinación de detectores de patrones 2D y clasificadores de alta precisión permite aspirar a pipelines de FRB \textbf{totalmente autónomos}, capaces de procesar en tiempo real los flujos masivos de datos de instrumentos nuevos (como CHIME, ASKAP, MeerKAT, FAST) y de activar alertas inmediatas para seguimiento multi-frecuencia.
...
\subsubsection{Componentes transversales de pipelines modernos}
...
\subsubsection{Perspectivas futuras: SKAO/ngVLA y algoritmos avanzados}
...
Además de los avances en algoritmos de detección, otro aspecto crucial del estado del arte en FRBs es la exploración de nuevas ventanas de frecuencia. Mientras que los métodos tradicionales y modernos han optimizado la detección en las bandas convencionales, la comunidad científica ha expandido significativamente el rango de frecuencias observadas, abriendo nuevas posibilidades para el estudio de estos fenómenos.
...
\subsection{Técnicas avanzadas de procesamiento y localización}

Complementando los avances en algoritmos de detección y exploración de frecuencias, el desarrollo de técnicas especializadas de procesamiento ha sido fundamental para el éxito de los pipelines modernos. Estas técnicas transversales abordan desafíos específicos que surgen en el procesamiento de datos de FRBs, desde la mitigación de interferencia hasta la localización precisa de eventos.
