\secnumbersection{Estado del Arte y Marco Conceptual}
...
Comprendiendo ya la naturaleza energética de los FRBs y la dicotomía repetidores/únicos, surge naturalmente la pregunta sobre estrategias observacionales óptimas. Históricamente, los FRBs se han observado en bandas centimétricas (0.1--10 GHz) donde exhiben mayor luminosidad pero sufren fuertemente dispersión y scattering ($\propto \nu^{-4}$, ver Ec.~\ref{eq:scattering_law}). Las observaciones milimétricas (30--300 GHz) mitigan estos efectos, permitiendo detectar pulsos intrínsecamente breves, aunque presentan desafíos de sensibilidad y campos de visión reducidos. Las secciones siguientes abordan: fundamentos físicos de los FRBs, casos emblemáticos de repetidores, conexión con magnetars (incluyendo SGR 1935+2154), detección en alta frecuencia, métodos tradicionales de búsqueda, y soluciones modernas de aprendizaje automático.
...
Un FRB típico se manifiesta como un pulso único de radio de duración brevísima ($\sim$1--10 ms) que sobresale muy significativamente por encima del ruido de fondo. En el dominio tiempo-frecuencia, la señal presenta la firma de la dispersión en plasma: las componentes de menor frecuencia arriban con mayor retraso temporal respecto a las de mayor frecuencia, según la ley de dispersión:

\begin{equation}
\Delta t \propto \mathrm{DM} \cdot \nu^{-2}
\label{eq:dispersion_law}
\end{equation}
...
Más allá de la fenomenología cualitativa, la DM proporciona información cuantitativa crucial. La medida de dispersión (DM) es una magnitud central en el estudio de FRBs. Se define como la integral de la densidad de electrones libres $n_e$ a lo largo de la línea de visión hasta la fuente:

\begin{equation}
\mathrm{DM} = \int_0^D n_e\, dl
\label{eq:dm_definition}
\end{equation}

\noindent típicamente expresada en unidades de pc cm$^{-3}$. Dado que las señales de radio sufren un retraso relativo dado por \citep{LorimerKramer2004}:

\begin{equation}
\Delta t \approx 4.15\,\mathrm{ms} \times \frac{\mathrm{DM}}{\mathrm{pc\,cm}^{-3}} \times\left[\left(\frac{\nu_{\mathrm{baja}}}{\mathrm{GHz}}\right)^{-2} - \left(\frac{\nu_{\mathrm{alta}}}{\mathrm{GHz}}\right)^{-2}\right]
\label{eq:dispersion_delay}
\end{equation}

\noindent una DM anómalamente alta es evidencia de un origen extragaláctico o cosmológico. Los FRBs descubiertos hasta ahora exhiben DM de cientos hasta miles de pc cm$^{-3}$, excediendo por mucho el contenido electrónico de la Vía Láctea en sus direcciones \citep{Petroff2019}. Por ejemplo, el FRB inicial de Lorimer tenía DM $\sim375$ pc cm$^{-3}$, varias veces mayor que la contribución galáctica esperada en su coordenada, implicando una distancia de orden gigapársec si el exceso de DM proviene del medio intergaláctico \citep{Lorimer2007}. La DM se utiliza entonces como un indicador de distancia cosmológica: aunque existe dispersión en la relación DM--$z$ por la distribución inhomogénea del medio intergaláctico, a primera aproximación DMs del orden $10^3$ pc cm$^{-3}$ sugieren orígenes a redshifts $z\sim0.5$--1 \citep{Inoue2004,Tendulkar2017}.

\subsubsection{Scattering y Ensanchamiento Temporal}

Además de la dispersión, los FRBs exhiben ensanchamiento temporal por scattering multipath en plasma turbulento, más pronunciado a bajas frecuencias \citep{Cordes2016}:

\begin{equation}
\tau_{\mathrm{sc}} \propto \nu^{-4}
\label{eq:scattering_law}
\end{equation}

A frecuencias altas, los pulsos son significativamente más breves: FRB 121102 mostró anchos de $\sim30$--100 $\mu$s a 4--8 GHz versus varios milisegundos a 1.4 GHz \citep{Gajjar2018a,Hessels2019,Nimmo2021}, motivando observaciones en bandas milimétricas para captar morfologías intrínsecas.
...
\caption{Figura~\ref{fig:morfologias_frb}. Cuatro arquetipos morfológicos de FRBs (CHIME/FRB, 400--800 MHz): (1) banda ancha simple, (2) banda estrecha simple, (3) temporalmente compleja, y (4) deriva descendente (``efecto trombón triste''). Cada panel muestra espectro dinámico (abajo) y perfil temporal (arriba). Crédito: \citet{Pleunis2021}.}
...
\caption{Figura~\ref{fig:frb121102_waterfall}. Espectros dinámicos de 18 ráfagas de FRB 121102 (DM = 560.5 pc cm$^{-3}$) en tres campañas: Arecibo (1.2--1.7 GHz), Green Bank (1.7--2.3 GHz), y GB-BL (4--8 GHz). Muestra deriva descendente característica (``efecto trombón triste'') y variabilidad morfológica. A frecuencias más altas (4--8 GHz), los pulsos son más breves ($\sim$30--100 $\mu$s), demostrando reducción del scattering. Adaptado de \citet{Hessels2019}.}
...
Más allá de las ráfagas múltiples, algunos repetidores exhiben periodicidad en su actividad. FRB 180916 (CHIME/FRB) reveló periodicidad de $\sim16.35$ días, la primera evidencia de ciclos de actividad en FRBs \citep{CHIME_FRB_Collaboration_2020}. Indicios tentativos de periodicidad $\sim161$ días también se reportaron para FRB 121102 \citep{Rajwade2020,cruces2020frb121102}, confirmada posteriormente con un período de $159.3 \pm 0.8$ días mediante monitoreo extendido en banda L \citep{2025A&A...693A..40B}. Estas modulaciones temporales sugieren sistemas binarios o precesión de magnetars, y son relevantes para estrategias de observación dirigida.
...
Sin embargo, las ventajas vienen acompañadas de desafíos significativos. Primero, los espectros típicamente decrecientes con índice espectral $\alpha \approx -1$ a $-3$ \citep{Jankowski2017}:
...
\noindent implican que pulsos a 100 GHz podrían ser 1--2 órdenes de magnitud más débiles que a 1 GHz. Un FRB de 1 Jy$\cdot$ms a 1.4 GHz tendría solo $\sim0.1$ Jy$\cdot$ms a 90 GHz.
...
\caption{Figura~\ref{fig:traditional_pipeline}. Pipeline tradicional de detección de FRBs. Flujo secuencial: (1) preprocesamiento/RFI, (2) dedispersión sobre parrilla DM (0--5000 pc cm$^{-3}$), (3) búsqueda de pulsos (filtros boxcar), (4) umbralización (S/N$>$6--8$\sigma$), y (5) clasificación/validación. Implementaciones modernas incorporan ML (ej. FETCH) en la etapa final. Adaptado de \citet{Wang2025CRACO}.}
...
\subsubsection{Gap Metodológico: Polarización como Firma de Validación en Alta Frecuencia}

Como se estableció en la Sección 2.5, el régimen milimétrico presenta desafíos algorítmicos fundamentales para la detección de FRBs. El desafío más crítico es la compresión drástica de la firma dispersiva: la curva $\nu^{-2}$ característica que permite distinguir FRBs de RFI en frecuencias centimétricas se vuelve casi imperceptible a frecuencias milimétricas, eliminando el indicador clave sobre el cual se basan tanto los pipelines tradicionales (Sección 2.6) como los modernos basados en deep learning como DRAFTS (Sección 2.7). Para FRBs extragalácticos sin DM conocido, detectar en mm sin firma dispersiva clara requiere estrategias alternativas de validación.

La evidencia observacional reciente proporciona una solución a este desafío. Como se discutió en la Sección 2.2.3, la polarización proporciona pistas cruciales sobre los entornos de los progenitores de FRBs, con muchos FRBs exhibiendo alta polarización lineal ($\sim100\%$) y medidas de rotación Faraday extremas. Más relevante aún para el régimen milimétrico, las observaciones pioneras del magnetar galáctico PSR J1745--2900 con ALMA a $\sim86$ GHz (Sección 2.5.4) revelaron que todos los pulsos detectados mostraron $>50\%$ de polarización lineal, claramente diferenciados de RFI no polarizada \citep{veracasanova2025}. Esta observación establece que los pulsos astrofísicos en alta frecuencia son altamente polarizados, proporcionando una firma de validación robusta que puede compensar la ausencia de firma dispersiva clara.

Por lo tanto, la polarización se convierte en una firma de validación crucial en alta frecuencia. Mientras que en bajas frecuencias la polarización puede ser una fuente de confusión (debido a variaciones en RM y depolarización), en el régimen milimétrico la alta polarización lineal de los pulsos astrofísicos genuinos contrasta marcadamente con la RFI típicamente no polarizada o débilmente polarizada. Esta propiedad física proporciona un criterio de validación ortogonal a la morfología dispersiva, permitiendo distinguir señales astrofísicas de artefactos instrumentales incluso cuando la firma dispersiva es imperceptible.

Sin embargo, los pipelines de estado del arte --tanto tradicionales (Sección 2.6) como basados en deep learning (Sección 2.7)-- no están diseñados para explotar la polarización como un clasificador morfológico primario. Los métodos tradicionales (PRESTO, HEIMDALL, CHIME) utilizan polarización principalmente para rechazo de RFI mediante umbrales de SNR en polarización lineal, pero no implementan clasificación morfológica en el dominio de polarización. DRAFTS, aunque representa el estado del arte en detección mediante deep learning, opera exclusivamente en el dominio de intensidad (Stokes I), utilizando el clasificador ResNet18 únicamente sobre espectros dinámicos de intensidad dedispersados. Ningún pipeline actual implementa clasificación morfológica mediante deep learning en polarización lineal (Stokes L) como un componente primario del proceso de validación.

Este gap metodológico establece la necesidad de extender DRAFTS hacia DRAFTS++, incorporando clasificación dual en intensidad y polarización lineal. La Línea 2b del Componente 2 (Capítulo 4) aborda precisamente este gap mediante la implementación de un clasificador morfológico de deep learning en polarización lineal, explotando la coherencia polarimétrica como criterio de validación primario en alta frecuencia. Esta extensión transforma la polarización de un criterio de rechazo secundario (como en pipelines tradicionales) a un clasificador morfológico primario que puede reducir dramáticamente los falsos positivos mediante validación en un dominio físico ortogonal, estableciendo así la justificación metodológica fundamental para la solución propuesta en esta tesis.
