...
Más allá de estos rasgos generales, los FRBs muestran una morfología espectro-temporal variada. Algunas ráfagas constan de uno solo pulso amplio en banda, mientras otras exhiben sub-pulsos múltiples que suelen derivar en frecuencia --un fenómeno coloquialmente llamado ``efecto trombón triste''-- donde sub-estallidos sucesivos aparecen a frecuencias menores con retrasos crecientes \citep{Hessels2019}. Este drift en frecuencia se ha observado sobre todo en FRBs repetidores, comenzando con FRB 121102 \citep{Gajjar2018b} y posteriormente en varias fuentes repetitivas \citep{CHIME_FRB_Collaboration_2020,Josephy2019}. Por el contrario, la mayoría de FRBs de evento único descubiertos con ASKAP muestran espectros puntuales e irregularmente modulados, concentrando la energía en rangos de frecuencia estrechos (pocos cientos de MHz) con poco o nulo drift apreciable \citep{Shannon2018,Cho2020}. Esta diferencia llevó a sugerir arquetipos morfológicos distintos para FRBs repetidores vs. no repetidores \citep{Pleunis2021}. Sin embargo, las comparaciones deben tomarse con cautela, ya que cada radiotelescopio y cada algoritmo de detección introducen sesgos; estudios recientes con muestras homogéneas (como el catálogo de CHIME) aún evalúan estadísticamente estas diferencias \citep{Fonseca2020,Pleunis2021}. La Figura~\ref{fig:morfologias_frb} ilustra los cuatro arquetipos morfológicos principales identificados por CHIME/FRB.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.92\textwidth]{Marco_conceptual/morfologias.jpg}
\caption{Cuatro arquetipos observados de morfología de FRBs detectados por CHIME/FRB en el rango 400--800 MHz. Panel 1: banda ancha simple (``simple broadband''), con emisión intensa distribuida en un amplio rango de frecuencias. Panel 2: banda estrecha simple (``simple narrowband''), con emisión concentrada en un rango limitado de frecuencias. Panel 3: temporalmente compleja (``temporally complex''), mostrando múltiples componentes dispersas en tiempo y frecuencia. Panel 4: deriva descendente (``downward drifting''), exhibiendo el característico ``efecto trombón triste'' donde sub-pulsos sucesivos aparecen a frecuencias decrecientes. Cada panel muestra el espectro dinámico (abajo) y el perfil temporal integrado (arriba). Las líneas horizontales blancas corresponden a canales enmascarados por RFI. Crédito: \citet{Pleunis2021}.}
\label{fig:morfologias_frb}
\end{figure}

Un ejemplo detallado de esta complejidad morfológica se observa en FRB 121102, cuyas múltiples ráfagas revelan patrones intrincados de deriva en frecuencia y sub-estructuras temporales, como se ilustra en la Figura~\ref{fig:frb121102_waterfall}.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.95\textwidth]{Marco_conceptual/waterfall-FRB121102.jpg}
\caption{Espectros dinámicos de 18 ráfagas del repetidor FRB 121102 dedispersadas a DM = 560.5 pc cm$^{-3}$. El panel superior de cada subgráfico muestra el perfil temporal integrado, mientras que el panel principal presenta el espectro dinámico frecuencia--tiempo. Las observaciones abarcan tres campañas: serie AO (Arecibo, 1.2--1.7 GHz), serie GB (Green Bank, 1.7--2.3 GHz) y GB-BL (Green Bank Breakthrough Listen, 4--8 GHz). Se aprecia claramente la deriva descendente de sub-pulsos (``downward drift'') característica de este repetidor, donde componentes sucesivas de cada ráfaga descienden sistemáticamente en frecuencia con el tiempo --el ``efecto trombón triste''--. Las marcas rojas horizontales indican canales de frecuencia enmascarados por interferencia de radiofrecuencia (RFI). Nótese la variabilidad morfológica entre ráfagas: algunas presentan un solo pulso amplio (AO-01, AO-10), mientras otras exhiben múltiples sub-pulsos nítidos y separados (AO-06, AO-11, GB-01, GB-02). A frecuencias más altas (GB-BL, 4--8 GHz), los pulsos son significativamente más breves ($\sim$30--100 $\mu$s), ilustrando la reducción del scattering con la frecuencia. Esta figura constituye un excelente ejemplo de la complejidad tiempo--frecuencia intrínseca de los FRBs repetidores. Adaptado de \citet{Hessels2019}, Fig. 1.}
...
La Figura~\ref{fig:traditional_pipeline} presenta un esquema detallado de este pipeline tradicional, mostrando el flujo completo desde los datos crudos hasta la lista final de candidatos.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.95\textwidth]{Marco_conceptual/PipelineClassic.png}
\caption{Pipeline tradicional de detección de FRBs. Esquema de flujo típico desde datos crudos hasta la lista de candidatos astrofísicos: (1) preprocesamiento con mitigación de RFI y calibración, (2) dedispersión coherente o incoherente sobre una parrilla densa de medidas de dispersión (típicamente decenas de miles de valores de DM entre 0--5000 pc cm$^{-3}$), (3) búsqueda de pulsos mediante filtros tipo boxcar de diferentes anchuras para maximizar la S/N, (4) umbralización y detección de candidatos con S/N$>$6--8$\sigma$, y (5) clasificación e inspección final de candidatos. Esta figura, adaptada del sistema de búsqueda CRACO (CRAFT Coherent upgrade) de ASKAP/CRAFT, ilustra claramente el encadenamiento secuencial: RFI $\rightarrow$ dedispersión $\rightarrow$ búsqueda de pulsos $\rightarrow$ umbralado $\rightarrow$ agrupamiento de candidatos $\rightarrow$ clasificación/validación. En implementaciones modernas, esta última etapa puede incorporar clasificadores de machine learning (como FETCH) para automatizar la discriminación entre FRBs reales y falsos positivos. Adaptado de \citet{Wang2025CRACO}.}
...
próxima generación en radioastronomía \citep{zhang2024drafts}. La Figura~\ref{fig:drafts_architecture} ilustra esquemáticamente la arquitectura de este pipeline innovador.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.95\textwidth]{WorkFlow-original.png}
\caption{Arquitectura del pipeline DRAFTS (Deep Learning-based RAdio Fast Transient Search). El sistema opera en dos etapas secuenciales: \textbf{Etapa 1 - Detección de Objetos:} Un detector basado en CenterNet analiza los datos transformados al espacio DM--tiempo (Medida de Dispersión vs. Tiempo) mediante aceleración CUDA, localizando candidatos potenciales de FRBs. La salida del CenterNet identifica las coordenadas centrales (DM óptima y tiempo de arribo) de cada evento candidato. \textbf{Etapa 2 - Clasificación Binaria:} Los candidatos detectados son extraídos de los datos originales en formato frecuencia--tiempo, preprocesados y alimentados a una red ResNet18 que evalúa su autenticidad mediante capas convolucionales con conexiones residuales (skip connections). La salida final es una probabilidad (sigmoid) que indica si el candidato corresponde a un FRB real (verde/rojo) o a ruido/RFI (azul). Esta arquitectura híbrida permite recuperar $\sim$3× más ráfagas que métodos tradicionales manteniendo tasas de falsos positivos mínimas. Adaptado de \citet{zhang2024drafts}.}
