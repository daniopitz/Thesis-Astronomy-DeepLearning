...
% (Se reubican Streaming y Precisión temporal en secciones correspondientes)

\subsubsection{Capacidades espectrales y polarización}
...
% Nota: Los impactos cuantitativos (p.ej., eficiencia y precisión temporal) se documentan en la sección de VALIDACIÓN DE LA SOLUCIÓN.

% (Sección de resultados y métricas del Bloque 1 movida a validación)
...
\end{tikzpicture}
\caption[Flujo del pipeline HF (TWL + fallback SNR)]{Flujo del Pipeline de Alta Frecuencia: detección híbrida TWL con fallback automático a SNR tradicional. El diagrama muestra la decisión que determina si usar detección híbrida (mapa t-W + CenterNet) o fallback a SNR tradicional. Fuente: Elaboración propia.}
