...
Un resultado particularmente notable de la Línea 2 corresponde a la detección de un **pulso único** del magnetar PSR J1745-2900 con morfología temporal excepcionalmente extendida, observado en slices consecutivos del archivo 2017-04-03-12\_47\_05\_0002. Este evento, obtenido con una ventana de análisis de 2 segundos y downsampling de 1080, presenta características que contrastan significativamente con pulsos típicos de magnetares, manifestándose como múltiples componentes temporales del mismo pulso principal.

El análisis de los slices consecutivos revela **13 componentes temporales** distribuidos en ~0.6 segundos, todos caracterizados por DM=46.9 pc cm$^{-3}$ y clasificación "BURST" con scores perfectos (1.00). El slice 036 (10.801-11.101s) presenta 7 componentes con SNR crudo de 6.1$\sigma$ y candidato optimizado de 7.3$\sigma$, mientras que el slice 037 (11.101-11.401s) muestra 6 componentes adicionales con SNR candidato de 8.3$\sigma$ y ancho temporal extendido de ~30.7 ms, considerablemente mayor que pulsos típicos.

La **secuencia temporal continua** de 13 componentes corresponde a un pulso único con estructura temporal excepcionalmente compleja, sugiriendo mecanismos físicos específicos: dispersión temporal intrínseca por plasma circumpulsar denso, scattering múltiple en el medio interestelar, o emisión con estructura de haz compleja. El análisis polarimétrico independiente (Figura \ref{fig:pulso_polarimetrico}) revela polarización lineal dominante (~13-14$\sigma$) sobre circular (~4-5$\sigma$), confirmando la naturaleza de emisión de púlsar con espectro dinámico de banda ancha uniforme (85.25-87.00 GHz).

Este resultado demuestra la capacidad excepcional del enfoque híbrido para **resolver estructura temporal fina** de pulsos con morfología extendida, **mantener especificidad alta** identificando correctamente múltiples componentes del mismo evento, y **capturar pulsos de ancho temporal atípico** que escapan a detectores basados exclusivamente en patrones dispersivos desarrollados. La capacidad de detectar un pulso único manifestado como 13 componentes temporales distintos representa un logro significativo en la caracterización de emisiones de magnetares en alta frecuencia.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{figures/DRAFTS-HF-SNR/2017-04-03-12_47_05_0002_slice036.png}
    \caption[Componentes temporales - Slice 036]{Detección de 7 componentes temporales del mismo pulso en slice 036 (10.801-11.101s). Mapa de detección DM-tiempo muestra componentes consistentes en DM=46.9 pc cm$^{-3}$ con clasificación perfecta (1.00). Waterfalls revelan SNR crudo 6.1$\sigma$, dedispersado 6.1$\sigma$, y candidato optimizado 7.3$\sigma$. La estructura temporal extendida sugiere dispersión temporal intrínseca o scattering múltiple en el medio circumpulsar. \textit{Fuente: Elaboración propia}.}
...
\begin{figure}[H]
    ...
    \caption[Componentes temporales - Slice 037]{Continuación temporal en slice 037 (11.101-11.401s) con 6 componentes adicionales del mismo pulso. Mantiene consistencia en DM=46.9 pc cm$^{-3}$ y clasificación perfecta. SNR candidato alcanza 8.3$\sigma$ con ancho temporal extendido ~30.7 ms, considerablemente mayor que pulsos típicos de magnetares. La secuencia completa (13 componentes en ~0.6s) corresponde a un pulso único con estructura temporal excepcionalmente compleja, evidenciando capacidad del sistema para resolver morfologías temporales finas. \textit{Fuente: Elaboración propia}.}
...
\begin{figure}[H]
    ...
    \caption[Análisis polarimétrico independiente]{Análisis polarimétrico del pulso principal confirmando naturaleza astrofísica. Panel superior: perfiles SNR temporal mostrando polarización lineal dominante (línea roja, ~13-14$\sigma$) sobre circular (línea azul, ~4-5$\sigma$). Panel inferior: espectro dinámico revelando emisión de banda ancha uniforme en 85.25-87.00 GHz. La asimetría polarimétrica es consistente con emisión de púlsar genuina en regímenes de alta frecuencia. \textit{Fuente: Elaboración propia}.}
