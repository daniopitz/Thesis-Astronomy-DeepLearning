...
\begin{algorithm}[H]
\caption{Integración y Ejecución de CenterNet en DRAFTS++}
\label{alg:centernet-detection}
\begin{algorithmic}[1]
\Require Imagen DM-tiempo $I$ del slice actual, modelo CenterNet pre-entrenado
\Ensure Lista de candidatos $\{(DM_i, t_i, \text{conf}_i)\}$ con coordenadas físicas

\State \textbf{INICIO}
\State \textbf{// Preprocesamiento de entrada}
\State Aplicar transformaciones estándar: normalización, redimensionamiento a 512×512, conversión a tensor
\State \textbf{// Inferencia con modelo pre-entrenado}
...
\State \textbf{// Post-procesamiento de salidas}
...
\State \textbf{// Conversión a coordenadas físicas}
...
\State \textbf{// Preprocesamiento robusto}
...
\State \textbf{// Decisión automatizada}
...
\subsubsection{Estrategia 2: Detección por SNR con clasificación binaria (Modelos Pre-entrenados)}

Cuando el retardo dispersivo se vuelve comparable o inferior a la resolución temporal, el patrón DM-tiempo pierde contraste y la representación más informativa pasa a ser el perfil temporal. Inspirada en el enfoque clásico de \textbf{PRESTO} \cite{2011ascl.soft07017R}, esta estrategia introduce una \textbf{rama SNR-threshold} dentro del \emph{workflow} de DRAFTS++: la \textit{propuesta de candidatos} se realiza mediante umbralización de SNR en la serie temporal (no mediante CenterNet); sin embargo, la \textit{clasificación final} se mantiene usando el mismo modelo ResNet18 pre-entrenado del Componente 1. 

\textbf{Contribución de ingeniería:} La rama SNR se \textit{activa condicionalmente} mediante un criterio físico automatizado: cuando el módulo de análisis determina que $\Delta t_{\mathrm{ms}} \le \alpha\, t_{\mathrm{samp}}$ (pérdida de firma bow-tie), el sistema conmuta automáticamente a detección por SNR. Tras proponer candidatos, el flujo \textit{regresa a la ruta estándar} del Componente 1: dedispersión local coherente, clasificación con ResNet18 pre-entrenado, validaciones físicas y emisión de artefactos estandarizados. Este diseño preserva (i) el carácter E2E del pipeline, (ii) la unificación de salidas y trazabilidad, y (iii) la reutilización completa de la infraestructura de clasificación y validación ya implementada, simplemente cambiando el método de propuesta inicial según el régimen de frecuencia.
...
\subsubsection{Transformación Arquitectónica y Operativa}
...
\subsubsection{Arquitectura Unificada: Principios y Flujo Operativo}

DRAFTS++ se fundamenta en tres principios arquitectónicos: \textbf{(1) Modularidad Estratégica} mediante patrón Strategy para intercambiar algoritmos según régimen observacional, \textbf{(2) Validación Física Unificada} mediante validador común aplicando criterios consistentes (DM válido, SNR umbral, coherencia temporal), y \textbf{(3) Visualización Desacoplada} generando artefactos estándar (CSV, plots, métricas).
...
\end{tikzpicture}}
\caption[Diagrama end-to-end DRAFTS++]{Diagrama end-to-end de DRAFTS++: Integración de Bloques 1 y 2, mostrando flujo desde inicialización hasta artefactos finales. La selección de estrategias en el diagrama incluye las 4 líneas exploratorias del Componente 2, aunque solo Líneas 1-2 están implementadas en esta memoria (Líneas 3-4 son extensiones arquitectónicas para trabajo futuro). \textit{Fuente: Elaboración propia}.}
\label{fig:pipeline-end-to-end}
\end{figure}

\medskip

\textit{Nota: Los diagramas detallados de flujo para Líneas 3 y 4 se omiten dado que estas líneas no fueron implementadas en esta memoria. Su diseño arquitectónico completo se desarrollará como parte del trabajo futuro de posgrado.}